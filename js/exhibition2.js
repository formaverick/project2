$(function () {
    $.ajax({
        url: "json/exhibition2.json",
        dataType: "json",
        success: function (data) {
            if (data.length > 0) {
                data.forEach((exhibition, i) => {
                    const $headInfo = $(".renewal_02 .ex1_1 .exhibition_head_info").eq(i);
                    const $mainImage = $(".renewal_02 .ex1_1 .re_main_image").eq(i);
                    ...
                    exhibition.product.forEach((product, j) => {
                        const k = i * 2 + j;
                        const $detail = $(".renewal_02 .re_detail").eq(k);
